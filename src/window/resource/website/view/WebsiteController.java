package window.resource.website.view;
...
public class WebsiteController {
    ...
    public void initialize(){
        ...
        setEditable(false);
        layout.setPrefWidth(Double.parseDouble(Window.widthProperty().getValue().toString())-200);
    ...
    private void updateDatabase(){
        ...
        try {
            String sql = "UPDATE web SET url = ?, email = ?, username = ?, password = ? WHERE user_id = ? AND name = ?";
    ...
    private void setValues(String value){
        ...
        try{
            PreparedStatement pst = connection.prepareStatement("SELECT * FROM web WHERE name = ? AND user_id = ?");
